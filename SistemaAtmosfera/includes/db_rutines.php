...
function get_single_channel_data($channel, $d1,$d2){
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		$data[]=array( $date->getTimestamp()*1000, intval($row[$channel]) );
	}
...
if(isset($_GET['action']) && !empty($_GET['action'])) {
	$action = $_GET['action'];
	switch($action) {
		case 'getDatatoPlot' : getDatatoPlot();break;
		case 'sendDataToFile' : sendDataToFile($_GET['d2'],$_GET['d1']);break;
		case 'getDatatoPlotMulti':getDatatoPlotMulti($_GET['channels_to_plot'],$_GET['d2'],$_GET['d1']);break;
		// ...etc...
	}
}
